...
public class ImageServiceImpl implements ImageService {
    ...
    public String uploadImage(MultipartFile contactImage, String filename) {
        // code to upload image on server/aws/cloud
        // we put on cloudinary , you can also put aws s3
    ...
    // method that return url of file
    @Override
    public String getUrlFromPublicId(String publicId) {

        return cloudinary.url().transformation(new Transformation<>()
                .width(AppConstants.CONTACT_IMAGE_WIDTH)
                .height(AppConstants.CONTACT_IMAGE_HEIGHT).crop(AppConstants.CONTACT_IMAGE_CROP)).generate(publicId);
    }
}
